#ifndef _VEML6030_FZ_H_
#define _VEML6030_FZ_H_
...
// 7 bit address (VEML6030_addr will be right shifted by 1 bit)
// so the address setting needs to be right shifted by 1 bit
#define VEML6030_addr 0x10 << 1

// extern void VEML6030_init();
...
private:
  ...
  // position.
  uint8_t _writeRegister(uint8_t _wReg, uint16_t _mask, uint16_t _bits, uint8_t _startPosition);
...
public:
  VEML6030_FZ(uint16_t address, I2C_HandleTypeDef * i2c_port);
  ~VEML6030_FZ();
  ...
  */
  ...
  // determined based on current gain and integration time settings. If the lux
  // value exceeds 1000 then a compensation formula is applied to it. 
  uint32_t readWhiteLight(void);
};

extern VEML6030_FZ VEML6030;

extern void init_veml6030(float gain, uint16_t integ_time);
